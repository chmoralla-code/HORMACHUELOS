...
const SESSION_ENV: &str = "HORMACHUELOS_DESKTOP_CU_SESSION";
const PAUSE_SENTINEL_ENV: &str = "HORMACHUELOS_DESKTOP_CU_PAUSE";
...
/// Runtime gate mirroring the persisted Settings opt-in. The helper refuses
/// input actions while this is false even if a stale session env survives.
...
fn ensure_not_paused() -> Result<()> {
        ...
        is_enabled(),
        "Desktop Computer Use is disabled. Enable it from Settings before continuing."
    );
    if is_paused() {
        crate::computer_fx::clear();
        bail!(
...
pub fn run_helper_if_requested() -> bool {
    ...
    let response = (|| -> Result<Value> {
            ...
            std::env::var(SESSION_ENV)
                ...
                .unwrap_or(false),
            "Computer Use helper may only be called by an active Hormachuelos session."
        );
...
mod platform {
    ...
    mod tests {
        ...
        fn protected_apps_are_blocked() {
            ...
            assert!(classify_blocked("Hormachuelos", "hormachuelos-optimized.exe").is_some());
